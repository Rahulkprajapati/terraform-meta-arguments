...
resource "google_compute_instance" "cloudclan-vm" {
  #UPDATE "each.value"  when calling variable "name" 
  name     = each.value
  for_each = toset(var.vm-name)
  #If your variable datatype is list then u need to add toset(var.vm-name) else if its string then only (var.vm-name)
  #NOTE: toset convert your list to set
